...
function FollowPage() {
    ...
    const HandleFollow = async (e) =>{
      try {
        e.preventDefault();
        let tempr = e.target.value;  
        const res = await fetch("http://localhost:3001/follow/person",{method: 'POST',
        headers: { 
          Accept: "application/json",
          "Content-Type": "application/json"
        },
        credentials: "include",
        body: JSON.stringify({username : tempr})      
        });
    ...
    const HandleUnFollow = async (e) =>{
      try {
        e.preventDefault();
        let tempr = e.target.value;  
        const res = await fetch("http://localhost:3001/unfollow/person",{method: 'POST',
        headers: { 
          Accept: "application/json",
          "Content-Type": "application/json"
        },
    ...
    const fetchUsers = async () =>{
      try {
           const apiurl = "http://localhost:3001/fetch/search";
